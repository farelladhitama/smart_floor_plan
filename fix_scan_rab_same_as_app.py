...
# ============================================================
# PATCH: Samakan estimasi_rab scan_floor_plans dengan RAB aplikasi
# File target:
# lib/app/modules/hasil_denah/controllers/hasil_denah_controller.dart
# ============================================================

path = Path(r"lib\app\modules\hasil_denah\controllers\hasil_denah_controller.dart")

if not path.exists():
    raise SystemExit("ERROR: hasil_denah_controller.dart tidak ditemukan. Jalankan patch ini dari root project smart_floor_plan.")

text = path.read_text(encoding="utf-8").replace("\r\n", "\n")

backup = path.with_suffix(path.suffix + ".before-fix-scan-rab-sama-aplikasi.bak")
backup.write_text(text, encoding="utf-8")

target = "  Future<double> _calculateScanEstimasiRab({"
start = text.find(target)

if start == -1:
    raise SystemExit("ERROR: method _calculateScanEstimasiRab tidak ditemukan.")

brace_start = text.find("{", start)

if brace_start == -1:
    raise SystemExit("ERROR: awal body method _calculateScanEstimasiRab tidak ditemukan.")
...
if end is None:
    raise SystemExit("ERROR: akhir body method _calculateScanEstimasiRab tidak ditemukan.")

new_method = '''  Future<double> _calculateScanEstimasiRab({
...
print("RAB scan sekarang dihitung memakai rumus volume bahan seperti halaman Estimasi RAB.")
